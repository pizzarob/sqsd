...
class SQSProcessor {
  constructor(options) {
    ...
    if (options.queueUrl) {
      ...
      options.queueName = options.queueUrl.substr(
        options.queueUrl.lastIndexOf("/") + 1
      );
    } else if (options.queueName) params.QueueName = options.queueName;
  ...
  async postToWorker(messageBody, sqsMessage) {
    ...
    if (
      ...
      sqsMessage.Attributes.ApproximateFirstReceiveTimestamp
    )
      headers["X-Aws-Sqsd-First-Received-At"] =
        sqsMessage.Attributes.ApproximateFirstReceiveTimestamp;

    if (sqsMessage.Attributes && sqsMessage.Attributes.ApproximateReceiveCount)
      headers["X-Aws-Sqsd-Receive-Count"] =
    ...
    return axios.post(this.options.webHook, Buffer.from(messageBody), {
      headers: headers,
  ...
  async handleMessage(sqsMessage) {
    const existingMsg = this.processingMesages.find(
      (msg) => msg.MD5OfBody === sqsMessage.MD5OfBody
    );
    if (existingMsg) {
      debug("MD5 exists in processing list. Returning.");
      return;
    }
    this.processingMesages.push(sqsMessage);
    debug(JSON.stringify(sqsMessage, void 0, 2));

    const messageBody = sqsMessage.Body;
    const receipt_handle = sqsMessage.ReceiptHandle;

    if (messageBody && messageBody.type === "BATCH_MINTED") {
      debug("Increase visibility timeout");
      await this._queue
    ...
    sqsMessage.promise = this.postToWorker(messageBody, sqsMessage)
      ...
      .then(async (postResult) => {
        debug(
          "Received result from worker, MessageId: %s  statusCode:%s ",
        ...
        if (!(postResult.status < 200 || postResult.status >= 300)) {
          await this._queue
            ...
            .promise();
          debug("Message successful removed from sqs, %o ", {
            MessageId: sqsMessage.MessageId,
            taskTime: new Date().getTime() - startTime,
          });
        } else
          error(
            "Worker respond  with status != 2XX, MessageId: %s  statusCode:%s ",
            sqsMessage.MessageId,
            postResult.status
          );
      })
      .then((x) => {
        debug(
          "Message successful processed,  MessageId: %s ",
          sqsMessage.MessageId
        );
      })
      .catch((err) => {
  ...
  async doCheckWorkerHealth(beginTimeStamp) {
    ...
    try {
      ...
      if (
        beginTimeStamp + this.options.workerHealthWaitTime <
        new Date().getTime()
      )
        return false;
      await delay(1000);
  ...
  async checkWorkerHealth() {
    if (this.healthChecked)
      //cached result, no sense to check worker on each cycle
      return this.healthChecked;
    if (!this.options.workerHealthUrl) return true;
  ...
  async tick() {
    if (
      this.polling ||
      this.processingMesages.length >= this.options.maxMessages
    ) {
      return;
    }

    debug(
      "Start Polling For %s Messages",
      this.options.maxMessages - this.processingMesages.length
    );
    this.polling = true;
    let data = await this._queue
      .receiveMessage({
        MaxNumberOfMessages:
          this.options.maxMessages - this.processingMesages.length,
    ...
    if (data && Array.isArray(data.Messages) && data.Messages.length > 0) {
      ...
      data.Messages.forEach((m) => {
        this.async handleMessage(m);
      });
    } else debug("No Messages Received via poll time");
